(function() {
  ...
  /*
   * Use gumshoe to set the "active" class and aria-active attribute on the
   * currently visible nav link. See the settings descriptions here:
   * <https://github.com/cferdinandi/gumshoe#global-settings>
   */
  var activeLink;
  gumshoe.init({
    selector: 'nav a',
    ...
    callback: function(data) {
      if (activeLink) {
        activeLink.setAttribute('aria-active', false);
      }
      activeLink = data.nav;
      if (activeLink) {
        activeLink.setAttribute('aria-active', true);
      }
    }
  });
